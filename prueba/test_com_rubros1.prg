...
poRepositorio = NEWOBJECT('com_rubros1', 'com_rubros1.prg')
...
separador()
? "Prueba: 3 | Método: 'nombre_existe' | Valor: 'repuestos'"
? 'Resultado esperado: pasar'
? 'Existe: ' + IIF(poRepositorio.nombre_existe('repuestos'), 'Sí', 'No')

IF !poRepositorio.nombre_existe('repuestos') THEN
    RETURN .F.
ENDIF
...
separador()
? "Prueba: 14 | Método: 'obtener_por_nombre' | Valor: 'repuestos'"
? 'Resultado esperado: pasar'
poModelo = poRepositorio.obtener_por_nombre('repuestos')
IF !imprimir() THEN
    RETURN .F.
ENDIF
