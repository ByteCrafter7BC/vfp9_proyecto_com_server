...
poRepositorio = NEWOBJECT('com_depar', 'com_depar.prg')
...
separador()
? "Prueba: 3 | Método: 'nombre_existe' | Valor: 'central'"
? 'Resultado esperado: pasar'
? 'Existe: ' + IIF(poRepositorio.nombre_existe('central'), 'Sí', 'No')

IF !poRepositorio.nombre_existe('central') THEN
    RETURN .F.
ENDIF
...
separador()
? "Prueba: 14 | Método: 'obtener_por_nombre' | Valor: 'boqueron'"
? 'Resultado esperado: pasar'
poModelo = poRepositorio.obtener_por_nombre('boqueron')
IF !imprimir() THEN
    RETURN .F.
ENDIF
