...
poRepositorio = NEWOBJECT('com_proceden', 'com_proceden.prg')
...
separador()
? "Prueba: 3 | Método: 'nombre_existe' | Valor: 'paraguay'"
? 'Resultado esperado: pasar'
? 'Existe: ' + IIF(poRepositorio.nombre_existe('paraguay'), 'Sí', 'No')

IF !poRepositorio.nombre_existe('paraguay') THEN
    RETURN .F.
ENDIF
...
separador()
? "Prueba: 14 | Método: 'obtener_por_nombre' | Valor: 'alemania'"
? 'Resultado esperado: pasar'
poModelo = poRepositorio.obtener_por_nombre('alemania')
IF !imprimir() THEN
    RETURN .F.
ENDIF
