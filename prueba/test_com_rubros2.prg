...
poRepositorio = NEWOBJECT('com_rubros2', 'com_rubros2.prg')
...
separador()
? "Prueba: 3 | Método: 'nombre_existe' | Valor: 'cigueñal'"
? 'Resultado esperado: pasar'
? 'Existe: ' + IIF(poRepositorio.nombre_existe('cigueñal'), 'Sí', 'No')

IF !poRepositorio.nombre_existe('cigueñal') THEN
    RETURN .F.
ENDIF
...
separador()
? "Prueba: 14 | Método: 'obtener_por_nombre' | Valor: 'cigueñal'"
? 'Resultado esperado: pasar'
poModelo = poRepositorio.obtener_por_nombre('cigueñal')
IF !imprimir() THEN
    RETURN .F.
ENDIF
