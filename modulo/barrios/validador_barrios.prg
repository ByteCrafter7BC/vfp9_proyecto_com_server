    ...
    **--------------------------------------------------------------------------
    PROTECTED FUNCTION validar_ciudad
        ...
        IF lnCiudad > 99999 THEN
            RETURN lcEtiqueta + STRTRAN(MENOR_QUE, '{}', ;
                ALLTRIM(STR(99999 + 1)))
        ENDIF

        loModelo = repositorio_obtener_por_codigo('ciudades', lnCiudad)

        IF VARTYPE(loModelo) != 'O' THEN
            RETURN lcEtiqueta + ;
                STRTRAN(NO_EXISTE, '{}', ALLTRIM(STR(lnCiudad)))
        ENDIF

        IF !loModelo.esta_vigente() THEN
            RETURN lcEtiqueta + ;
                STRTRAN(NO_VIGENTE, '{}', ALLTRIM(STR(lnCiudad)))
        ENDIF
