...
poRepositorio = NEWOBJECT('com_barrios', 'com_barrios.prg')
...
separador()
? "Prueba: 3 | Método: 'existe_nombre' | Valor: 'centro', 9, 115"
? 'Resultado esperado: pasar'
? 'Existe: ' + IIF(poRepositorio.existe_nombre('centro', 9, 115), 'Sí', 'No')

IF !poRepositorio.existe_nombre('centro', 9, 115) THEN
    RETURN .F.
ENDIF

separador()
? "Prueba: 4 | Método: 'existe_nombre' | Valor: 'monark', 10, 12"
? 'Resultado esperado: fallar'
? 'Existe: ' + IIF(poRepositorio.existe_nombre('monark', 10, 12), 'Sí', 'No')

IF poRepositorio.existe_nombre('monark', 10, 12) THEN
    RETURN .F.
ENDIF
...
separador()
? "Prueba: 14 | Método: 'obtener_por_nombre' | Valor: 'domingo savio', 12, 154"
? 'Resultado esperado: pasar'
poModelo = poRepositorio.obtener_por_nombre('domingo savio', 12, 154)
IF !imprimir() THEN
    RETURN .F.
ENDIF

separador()
? "Prueba: 15 | Método: 'obtener_por_nombre' | Valor: 'monark', 10, 12"
? 'Resultado esperado: fallar'
poModelo = poRepositorio.obtener_por_nombre('monark', 10, 12)
IF imprimir() THEN
    RETURN .F.
ENDIF
...
FUNCTION imprimir
    ...
    WITH poModelo
        ...
        ? 'Departamento: ' + ALLTRIM(STR(.obtener_departamen()))
        ? 'Ciudad: ' + ALLTRIM(STR(.obtener_ciudad()))
