...
poRepositorio = NEWOBJECT('com_cobrador', 'com_cobrador.prg')
...
separador()
? "Prueba: 3 | Método: 'existe_nombre' | Valor: 'alvarez, felipe'"
? 'Resultado esperado: pasar'
? 'Existe: ' + IIF(poRepositorio.existe_nombre('alvarez, felipe'), 'Sí', 'No')

IF !poRepositorio.existe_nombre('alvarez, felipe') THEN
    RETURN .F.
ENDIF
...
separador()
? "Prueba: 14 | Método: 'obtener_por_nombre' | Valor: 'nuñez, oscar'"
? 'Resultado esperado: pasar'
poModelo = poRepositorio.obtener_por_nombre('nuñez, oscar')
IF !imprimir() THEN
    RETURN .F.
ENDIF
